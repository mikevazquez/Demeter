...
describe("SF-240 student Rewards portal", () => {
  ...
  it("does not present cancelled or expired reward progress as active", () => {
    ...
    expect(isStudentRewardProgressActive(participation, activeRule, openFutureCycle, now)).toBe(
      true,
    );
    expect(isStudentRewardProgressActive(participation, cancelledRule, openFutureCycle, now)).toBe(
      false,
    );
    ...
    expect(isStudentRewardProgressFinalized(participation, activeRule, openExpiredCycle, now)).toBe(
      true,
    );
  });
