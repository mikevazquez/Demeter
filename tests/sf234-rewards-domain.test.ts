...
describe("SF-234 rewards domain", () => {
  it(
    "locks the approved families, lifecycle and frozen reward contract",
    () => {
      const sql = readFileSync(
        join(
          process.cwd(),
          "supabase/migrations/20260919200000_sf234_rewards_domain.sql",
        ),
        "utf8",
      );
      ...
      expect(sql).toContain(
        "Rule changes never rewrite an already generated reward",
      );
    },
  );
  ...
  it("keeps progress snapshots separate from source-of-truth tables", () => {
    ...
    expect(sql).not.toContain(
      "alter table public.reservations add column reward_progress",
    );
    expect(sql).not.toContain(
