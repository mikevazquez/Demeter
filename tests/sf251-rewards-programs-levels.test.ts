...
describe("SF-251 Progress & Rewards V1 programs and levels", () => {
  ...
  it("adds a versioned program domain above immutable reward rule versions", () => {
    for (const table of [
      "reward_programs",
  ...
  it("preserves published history while allowing only unpublished drafts to change", () => {
  ...
  it("materializes zero-progress students and keeps permanent level unlocks", () => {
    ...
    expect(rpc).toContain("v_participation.current_level_order is distinct from v_level.level_order");
    expect(rpc).toContain("'program_completed'");
  });

  it("reuses the existing outcomes engine for zero, one or many outcomes", () => {
    expect(runtime).toContain("jsonb_typeof(v_version.reward_definition->'rewards') = 'array'");
    expect(runtime).toContain("jsonb_array_elements(v_version.reward_definition->'rewards')");
  ...
  it("keeps student reads scoped through Rewards RLS", () => {
    expect(domain).toContain("private.has_capability(studio_id, 'rewards.read')");
    expect(domain).toContain("private.is_reward_student_self(studio_id, student_id)");
