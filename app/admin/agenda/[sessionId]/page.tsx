...
export default async function SessionDetailPage({
  ...
  const { data: session } = await supabase
    .from("class_sessions")
    .select("id, template_id, starts_at, ends_at, capacity, status, location_id")
    .eq("id", sessionId)
  ...
  const [
    ...
    supabase
      ...
      .single(),
    session.location_id
      ? supabase.from("studio_locations").select("name").eq("id", session.location_id).single()
    ...
    supabase
      .from("reservations")
      .select("id, student_id, status, student_package_id")
      .eq("session_id", sessionId)
      .neq("status", "cancelled")
      .order("booked_at"),
    supabase
      .from("student_packages")
      .select("id, student_id, credits_remaining, starts_on, expires_on")
      .eq("studio_id", membership.studio_id),
  ]);
  ...
  const eligibleStudentIds = new Set(
    (assignments ?? [])
      .filter(
        (item) =>
          item.student_id &&
          item.starts_on <= classDate &&
          item.expires_on >= classDate &&
          (item.credits_remaining === null || item.credits_remaining > 0),
      )
    ...
    <main className="dashboard-shell">
      ...
      {query.error ? (
        <div className="notice error">
          No se pudo completar la reserva. Verifica paquete, vigencia y capacidad.
        </div>
      ) : null}
      ...
      <section className="stat-grid">
        ...
        <article className="stat-card">
          <span>Estado</span>
          <strong className="stat-word">{session.status}</strong>
          ...
          {!canEdit ? (
            ...
            <form action={bookStudent} className="compact-form reservation-form">
              ...
              <button
                ...
                type="submit"
                disabled={eligibleStudents.length === 0 || available === 0}
              >
                Reservar lugar
              </button>
              {eligibleStudents.length === 0 ? (
                <p className="form-help">
                  No hay alumnas disponibles con paquete vigente y créditos.
                </p>
              ) : null}
            </form>
          )}
