...
export default async function AgendaPage({
  ...
  const [
    ...
    supabase
      .from("class_sessions")
      .select(
        "id, starts_at, ends_at, capacity, status, notes, template_id, space_id, instructor_id",
      )
      .eq("studio_id", studio.id)
  ...
  const errorCopy: Record<string, string> = {
    conflict: "El instructor o espacio ya está ocupado en uno de los horarios.",
    space: "El espacio no es válido o su capacidad es menor al cupo de la clase.",
    ...
    <main className="dashboard-shell">
      <header className="topbar">
        <div>
          ...
          <p>Disciplinas, actividades y sesiones reales del estudio.</p>
        </div>
      </header>
      {params.created ? (
        <div className="notice success">
          {params.created === "series"
            ? "Serie semanal programada correctamente."
            : "Cambio guardado correctamente."}
        </div>
      ) : null}
      {params.error ? (
        <div className="notice error">
          {errorCopy[params.error] ?? "No se pudo guardar. Revisa los datos e inténtalo de nuevo."}
        </div>
      ) : null}
        ...
        <aside className="agenda-sidebar">
          {!canEdit ? (
            ...
            <>
              ...
              <article className="panel compact-panel">
                <p className="eyebrow">3 · SESIÓN</p>
                ...
                <form action={createSession} className="compact-form">
                  ...
                  <select name="instructor_id" defaultValue="">
                    ...
                    {instructors?.map((item) => (
                      <option key={item.id} value={item.id}>
                        {personMap.get(item.person_id) ?? "Instructor"}
                      </option>
                    ))}
                  </select>
                  <select name="space_id" defaultValue={spaces?.[0]?.id ?? ""}>
                    <option value="">Sin espacio</option>
                    {spaces?.map((item) => (
                      <option key={item.id} value={item.id}>
                        {item.name}
                        {item.capacity ? ` · máx. ${item.capacity}` : ""}
                      </option>
                    ))}
                  </select>
                  <input
