...
export async function createProduct(formData: FormData) {
  ...
  const creditLimit = unlimited ? null : parsePositiveInt(formData.get('credit_limit'), 'credit_limit');
  ...
  if (disciplineIds.length) {
    const { error: disciplineError } = await ctx.supabase.from('product_template_disciplines').insert(
