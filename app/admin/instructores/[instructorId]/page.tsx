...
export default async function InstructorProfilePage({ params, searchParams }: { params: Promise<{ instructorId: string }>; searchParams: Promise<{ created?: string; saved?: string; error?: string }> }) {
  ...
  const { data: instructor } = await supabase.from("instructors").select("id, person_id, status, bio, created_at").eq("id", instructorId).eq("studio_id", studio.id).maybeSingle();
  if (!instructor) notFound();

  const [{ data: person }, { data: contacts }, { data: links }, { data: disciplines }] = await Promise.all([
  ...
  return (
    <main className="dashboard-shell">
      ...
      {query.created ? <div className="notice success">Instructor creado correctamente.</div> : null}
      ...
      <section className="panel-grid">
        ...
        <article className="panel"><p className="eyebrow">ACCESO</p><h2>Cuenta separada</h2><div className="empty-state">Este perfil no crea credenciales automáticamente. La vinculación de UserAccount + membership INSTRUCTOR se realiza mediante el flujo de acceso, sin convertir el perfil operativo en una cuenta.</div></article>
      </section>
      ...
      <section className="panel"><p className="eyebrow">AGENDA</p><h2>Clases asignadas</h2><div className="empty-state">La asignación de instructor a Session se conecta en F5 para mantener una sola fuente de agenda.</div></section>
