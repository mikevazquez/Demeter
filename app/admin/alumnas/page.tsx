...
export default async function StudentsPage({
  ...
  const [{ data: studio }, { data: students }, { data: packages }, { data: assignments }] =
    await Promise.all([
      ...
      supabase
        .from("packages")
        .select("id, name, class_credits, validity_days, price_cents, active")
        .eq("studio_id", membership.studio_id)
        .order("name"),
      supabase
        .from("student_packages")
        .select(
          "id, student_id, package_id, credits_remaining, credits_total, starts_on, expires_on",
        )
        .eq("studio_id", membership.studio_id)
  ...
  const assignmentsByStudent = new Map<
    string,
    typeof assignments extends (infer T)[] | null ? T[] : never
  >();
      ...
      <section className="agenda-layout">
        <div className="agenda-main">
          <article className="panel">
            ...
            {(students?.length ?? 0) === 0 ? (
              <div className="empty-state">
                Aún no hay alumnas. Crea la primera desde el panel de la derecha.
              </div>
            ) : (
              <div className="student-list">
                {students?.map((student) => {
                  const activeAssignments = (assignmentsByStudent.get(student.id) ?? []).filter(
                    (item) => item.expires_on >= today,
                  );
                  ...
                  return (
                    <div className="student-row" key={student.id}>
                      ...
                      <div className="student-package-summary">
                        ...
                        <span>
                          {current
                            ? `${current.credits_remaining ?? "∞"} clases · vence ${current.expires_on}`
                            : "Asigna un paquete para reservar"}
                        </span>
                      </div>
                    </div>
                  );
          ...
          <article className="panel">
            <div className="panel-heading">
              ...
              <span className="count-badge">{packages?.length ?? 0}</span>
            </div>
            <div className="package-grid">
              {packages?.map((item) => (
                <div className="package-card" key={item.id}>
                  ...
                  <span>
                    {item.class_credits ?? "Ilimitadas"} clases · {item.validity_days} días
                  </span>
        ...
        <aside className="agenda-sidebar">
          {!canEdit ? (
            <article className="panel">
              <p>
                Tu rol puede consultar alumnas y paquetes, pero solo owner y admin pueden
                modificarlos.
              </p>
            </article>
          ) : (
            <>
              <article className="panel compact-panel">
                ...
                <form action={createStudent} className="compact-form">
                  ...
                  <small id="student-phone-help">
                    Obligatorio. Se guarda como +52 y será el identificador de acceso de la alumna.
                  </small>
                  <input
                    name="email"
                    type="email"
                ...
                <form action={createPackage} className="compact-form">
                  ...
                  <div className="form-split">
                    <input
                      ...
                      min="1"
                      placeholder="Clases"
                      aria-label="Créditos; vacío para ilimitado"
                    />
                    <input
                      name="validity_days"
                      type="number"
              ...
              <article className="panel compact-panel">
                ...
                <form action={assignPackage} className="compact-form">
                  ...
                  <select name="package_id" required defaultValue="">
                    <option value="" disabled>
                      Paquete
                    </option>
                    {packages
                  ...
                  <button
                    ...
                    type="submit"
                    disabled={!students?.length || !packages?.length}
                  >
                    Asignar paquete
                  </button>
                </form>
