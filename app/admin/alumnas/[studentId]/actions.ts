...
export async function grantBronzeMedal(formData: FormData) {
  const studentId = String(formData.get("student_id") ?? "").trim();
  const reason = String(formData.get("reason") ?? "").trim();

  if (!studentId || !reason) {
    redirect(`/admin/alumnas/${studentId}?view=rewards&error=bronze_reason`);
  }
  ...
  if (error) {
    redirect(`/admin/alumnas/${studentId}?view=rewards&error=bronze_grant`);
  }
  ...
  redirect(`/admin/alumnas/${studentId}?view=rewards&saved=bronze_granted`);
}
