...
export default async function AdminPage() {
  ...
  const { data: studio } = await supabase.from("studios").select("name, timezone").eq("id", membership.studio_id).single();
  const timeZone = studio?.timezone ?? "America/Mexico_City";

  const todayParts = new Intl.DateTimeFormat("en-CA", { timeZone, year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(new Date());
  const part = (type: string) => todayParts.find((item) => item.type === type)?.value ?? "";
  const todayLocal = `${part("year")}-${part("month")}-${part("day")}`;
  const offsetName = new Intl.DateTimeFormat("en-US", { timeZone, timeZoneName: "longOffset", hour: "2-digit" }).formatToParts(new Date()).find((item) => item.type === "timeZoneName")?.value ?? "GMT-06:00";
  ...
  const [{ data: sessions }, { count: activeStudents }] = await Promise.all([
    supabase.from("class_sessions").select("id, starts_at, ends_at, capacity, status, template_id").eq("studio_id", membership.studio_id).gte("starts_at", start.toISOString()).lt("starts_at", end.toISOString()).order("starts_at", { ascending: true }),
    supabase.from("students").select("*", { count: "exact", head: true }).eq("studio_id", membership.studio_id).eq("active", true),
  ]);

  const sessionIds = (sessions ?? []).map((session) => session.id);
  const { count: reservationsCount } = sessionIds.length
    ? await supabase.from("reservations").select("*", { count: "exact", head: true }).in("session_id", sessionIds).eq("status", "booked")
    : { count: 0 };
  ...
  return (
    <main className="dashboard-shell">
      <header className="topbar">
        <div><p className="eyebrow">ADMINISTRACIÓN</p><h1 className="dashboard-title">Hoy en {studio?.name ?? "tu estudio"}</h1></div>
        <div className="toolbar-actions">
          <Link className="secondary-button" href="/admin/alumnas">Alumnas</Link>
      ...
      <section className="stat-grid">
        ...
        <article className="stat-card"><span>Reservas hoy</span><strong>{reservationsCount ?? 0}</strong><small>En clases de hoy</small></article>
      ...
      <section className="panel-grid">
        <article className="panel">
          ...
          {(sessions?.length ?? 0) === 0 ? <div className="empty-state">Todavía no hay clases programadas para hoy.</div> : (
            <div className="session-list">{sessions?.map((session) => (
              <Link className="session-row session-link" href={`/admin/agenda/${session.id}`} key={session.id}>
        ...
        <article className="panel"><p className="eyebrow">ACCESO</p><h2>Rol actual</h2><div className="role-pill">{membership.role}</div><p>La sesión está protegida por Supabase Auth y las reglas RLS de Studio Flow.</p></article>
      </section>
    </main>
  );
}
