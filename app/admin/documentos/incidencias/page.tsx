...
export default async function DocumentIncidentsPage({
  ...
  const [
    ...
    supabase
      ...
      .select(
        "id,student_id,code,title,detail,action_kind,starts_at,expires_at,resolved_at,created_at",
      )
      .eq("studio_id", studio.id)
      .order("created_at", { ascending: false })
      .limit(30),
    ...
    <main className="dashboard-shell space-y-5">
      ...
      {can(CAPABILITIES.DOCUMENTS_MANAGE) ? (
        <section className="grid gap-4 xl:grid-cols-3">
          <form
            action={recordExternalAcceptanceAction}
          ...
          >
            ...
            <div className="mt-4 space-y-3">
              <select
                name="student_id"
                required
                className="w-full rounded-xl border border-white/10 bg-[#11131b] px-3 py-2.5 text-sm text-white"
              >
              ...
              <input
                name="reference"
                placeholder="Referencia / folio opcional"
              ...
              <textarea
                ...
                minLength={3}
                rows={3}
                placeholder="Motivo y contexto obligatorio"
                className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white"
              />
          ...
          <form
            action={addBookingRestrictionAction}
            className="rounded-3xl border border-amber-400/20 bg-[#0d0f16] p-5"
          >
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-300">
              Crear restricción
            </p>
            <p className="mt-2 text-xs leading-5 text-zinc-500">
              Úsala solo para adeudos, penalizaciones o condiciones que realmente deban impedir
              nuevas reservas.
            </p>
            <div className="mt-4 space-y-3">
              <select
              ...
              <input
                name="code"
                required
                placeholder="Código · ej. overdue_installment"
                className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white"
              />
              <input
                name="title"
                required
                placeholder="Título visible para la alumna"
                className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white"
              />
              <textarea
                name="detail"
                rows={3}
                placeholder="Qué pasó y qué necesita hacer"
                className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white"
              />
              <select
                name="action_kind"
          ...
          <form
            action={grantBookingExceptionAction}
            className="rounded-3xl border border-cyan-400/20 bg-[#0d0f16] p-5"
          >
            ...
            <div className="mt-4 space-y-3">
              ...
              <input
                ...
                placeholder="Código específico o vacío para excepción general"
                className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white"
              />
              <input
                name="source_id"
                placeholder="ID de restricción opcional"
                className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white"
              />
              <textarea
                name="reason"
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <strong className="text-sm text-white">
                        {studentMap.get(item.student_id) ?? "Alumna"}
                      </strong>
                      <p className="mt-1 text-sm text-zinc-300">{item.title}</p>
                      <p className="mt-1 text-xs text-zinc-500">{item.detail || item.code}</p>
                    </div>
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.resolved_at ? "bg-emerald-400/10 text-emerald-300" : "bg-amber-400/10 text-amber-200"}`}
                    >
                  ...
                  {!item.resolved_at && can(CAPABILITIES.DOCUMENTS_MANAGE) ? (
                    <form action={resolveBookingRestrictionAction} className="mt-3 flex gap-2">
                      ...
                      <input
                        ...
                        className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-white"
                      />
                      <button className="rounded-xl border border-emerald-400/25 px-3 py-2 text-xs font-semibold text-emerald-200">
                        Resolver
                      </button>
                    </form>
                  ) : null}
              ...
              exceptions?.map((item) => (
                <div key={item.id} className="px-5 py-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.revoked_at ? "bg-zinc-500/10 text-zinc-400" : new Date(item.expires_at) <= new Date() ? "bg-zinc-500/10 text-zinc-400" : "bg-cyan-400/10 text-cyan-200"}`}
                    >
                      {item.revoked_at
                        ? "Revocada"
                  ...
                  can(CAPABILITIES.DOCUMENTS_MANAGE) ? (
                    <form action={revokeBookingExceptionAction} className="mt-3 flex gap-2">
                      ...
                      <input
                        ...
                        className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-white"
                      />
                      <button className="rounded-xl border border-rose-400/25 px-3 py-2 text-xs font-semibold text-rose-200">
                        Revocar
                      </button>
                    </form>
                  ) : null}
        ...
        <div className="divide-y divide-white/10">
          {(incidents ?? []).length ? (
            incidents?.map((item) => (
              <div
                key={item.id}
                className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_.7fr_auto] sm:items-center"
              >
                <div>
                  <strong className="text-sm text-white">
                    {studentMap.get(item.student_id ?? "") ?? "Caso general"}
                  </strong>
                  <p className="mt-1 text-xs text-zinc-500">{item.reason}</p>
                </div>
