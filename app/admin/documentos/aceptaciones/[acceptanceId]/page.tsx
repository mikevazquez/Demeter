...
export default async function AcceptanceEvidencePage({
  ...
  const { data: acceptance } = await supabase
    ...
    .select(
      "id,student_id,version_id,acceptor_kind,guardian_id,decision,method,affirmation_text,evidence,actor_user_id,accepted_at",
    )
    .eq("id", acceptanceId)
  ...
  const [{ data: student }, { data: version }, { data: guardian }, { data: invalidation }] =
    await Promise.all([
      supabase
        ...
        .eq("id", acceptance.student_id)
        .maybeSingle(),
      supabase
        .from("document_versions")
        .select(
          "id,document_id,version_number,file_path,file_name,file_size_bytes,content_sha256,response_mode,acceptance_party",
        )
        .eq("id", acceptance.version_id)
        .maybeSingle(),
      acceptance.guardian_id
        ? supabase
            .from("student_guardians")
            .select("id,full_name,relationship,relationship_detail")
            .eq("id", acceptance.guardian_id)
            .maybeSingle()
        : Promise.resolve({ data: null }),
      supabase
        .from("document_acceptance_invalidations")
        .select("reason,invalidated_at,invalidated_by")
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-3xl border border-emerald-400/20 bg-[#0d0f16] p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <span
                className={`rounded-full px-3 py-1 text-xs font-semibold ${invalidation ? "bg-rose-400/10 text-rose-200" : "bg-emerald-400/10 text-emerald-300"}`}
              >
                {invalidation
                  ? "Invalidada"
                  : acceptance.decision === "declined"
                    ? "No autorizado"
                    : "Aceptada"}
              </span>
            </div>
            ...
            <dl className="mt-5 grid gap-4 sm:grid-cols-2">
              <div>
                <dt className="text-xs text-zinc-500">Aceptante</dt>
                <dd className="mt-1 text-sm font-semibold text-white">
                  {guardian?.full_name || student.full_name}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-zinc-500">Tipo</dt>
                <dd className="mt-1 text-sm font-semibold text-white">
                  {acceptance.acceptor_kind === "guardian" ? "Responsable" : "Alumna"}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-zinc-500">Fecha y hora</dt>
                <dd className="mt-1 text-sm font-semibold text-white">
              ...
              <div>
                <dt className="text-xs text-zinc-500">Método</dt>
                <dd className="mt-1 text-sm font-semibold text-white">
                  {acceptance.method.replaceAll("_", " ")}
                </dd>
              </div>
              {guardian ? (
                <div>
                  <dt className="text-xs text-zinc-500">Relación declarada</dt>
                  <dd className="mt-1 text-sm font-semibold text-white">
              ...
              <div>
                ...
                <dd className="mt-1 text-sm font-semibold text-white">{acceptance.decision}</dd>
              </div>
            </dl>
            ...
            <div className="rounded-3xl border border-rose-400/20 bg-rose-400/[0.04] p-5">
              ...
              <p className="mt-2 text-xs text-zinc-500">
                {new Intl.DateTimeFormat("es-MX", {
                  ...
                  timeStyle: "short",
                  timeZone: studio.timezone,
                }).format(new Date(invalidation.invalidated_at))}
              </p>
            </div>
          ) : null}
        </div>
        ...
        <aside className="space-y-4">
          <div className="rounded-3xl border border-fuchsia-500/20 bg-[#0d0f16] p-5">
            ...
            <h3 className="mt-3 text-lg font-semibold text-white">
              {version.file_name || document.name}
            </h3>
            ...
            {signedUrl ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-4 inline-flex w-full justify-center rounded-xl border border-white/15 px-4 py-2 text-sm font-semibold text-white"
              >
                Ver versión exacta
              </a>
            ) : null}
          ...
          {can(CAPABILITIES.DOCUMENTS_MANAGE) && !invalidation ? (
            <form
              action={invalidateAcceptanceAction}
              className="rounded-3xl border border-rose-400/20 bg-[#0d0f16] p-5"
            >
