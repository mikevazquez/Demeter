...
export default async function RewardIncidentsPage({
  ...
  let request = ctx.supabase
    ...
    .select(
      "id,student_id,rule_id,reward_instance_id,incident_type,priority,status,summary,opened_at,updated_at",
    )
  ...
  const studentIds = [
    ...new Set((incidents ?? []).map((item) => item.student_id).filter(Boolean)),
  ] as string[];
    ...
    <main className="dashboard-shell space-y-6">
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-rose-500/20 bg-rose-500/[0.05] p-4">
          <p className="text-xs text-rose-300">Críticas</p>
          <strong className="mt-2 block text-2xl text-white">{critical}</strong>
        </div>
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.05] p-4">
          <p className="text-xs text-amber-300">En revisión</p>
          <strong className="mt-2 block text-2xl text-white">{inReview}</strong>
        </div>
      </section>
            ...
            >
              <div>
                    ...
                    className={
                      incident.priority === "critical"
                        ...
                        : incident.priority === "high"
                          ...
                          : "rounded-full bg-zinc-500/15 px-2.5 py-1 text-xs font-semibold text-zinc-300"
                    }
                  >
                ...
                <p className="mt-1 text-xs text-zinc-500">
                  {studentMap.get(incident.student_id ?? "") ?? "Contexto de sistema"} ·{" "}
