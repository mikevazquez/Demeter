...
export default async function RewardsHomePage() {
  ...
  const [rulesResult, participationsResult, rewardsResult, incidentsResult, lifecycleResult, rewardEventsResult, cyclesResult] =
    await Promise.all([
      ctx.supabase
        .from("reward_rules")
  ...
  return (
    <main className="dashboard-shell space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <Link href="/admin/empresa" className="mb-3 inline-flex text-sm font-semibold text-zinc-400 transition hover:text-white">
            ← Empresa
          </Link>
        ...
        {canManage ? (
          <Link href="/admin/recompensas/reglas/nueva" className="rounded-xl bg-fuchsia-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-fuchsia-500">
            + Crear regla
          </Link>
        ) : null}
      </header>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Reglas activas" value={activeRules.length} detail={`${rules.length} totales`} />
        <MetricCard label="Alumnas participando" value={participatingStudents} detail="participación elegible o activa" />
        <MetricCard label="Recompensas disponibles" value={availableRewards.length} detail="listas para utilizar" />
        <MetricCard
          label="Incidencias abiertas"
          value={openIncidents.length}
          detail={openIncidents.some((item) => item.priority === "critical") ? "hay prioridad crítica" : "sin bloqueos operativos"}
        />
      </section>
      ...
      {openIncidents.length ? (
        <section className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.06] p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.15em] text-amber-300">REQUIERE ATENCIÓN</p>
              <h2 className="mt-1 text-lg font-semibold text-white">
                {openIncidents.length} incidencia{openIncidents.length === 1 ? "" : "s"} abierta{openIncidents.length === 1 ? "" : "s"}
              </h2>
            </div>
            <Link href="/admin/recompensas/incidencias" className="text-sm font-semibold text-amber-200 hover:text-white">
              Revisar incidencias →
            </Link>
          </div>
        </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
        <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.15em] text-fuchsia-300">REGLAS DESTACADAS</p>
              <h2 className="mt-1 text-xl font-semibold text-white">Activas ahora</h2>
            </div>
            <Link href="/admin/recompensas/reglas" className="text-sm font-semibold text-zinc-400 hover:text-white">Ver todas →</Link>
          </div>

          <div className="mt-5 grid gap-3">
            {!activeRules.length ? (
              <EmptyState title="Todavía no hay reglas activas">Crea una regla, revísala y actívala cuando esté lista.</EmptyState>
            ) : (
              activeRules.slice(0, 5).map((rule) => {
                const version = versionMap.get(`${rule.id}:${rule.current_version_number}`);
                return (
                  <Link key={rule.id} href={`/admin/recompensas/reglas/${rule.id}`} className="rounded-2xl border border-white/10 bg-black/15 p-4 transition hover:border-fuchsia-500/35">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <h3 className="mt-1 font-semibold text-white">{String(version?.name ?? "Regla")}</h3>
                        <p className="mt-1 text-sm text-zinc-400">{String(version?.human_summary ?? "")}</p>
                      </div>
                      <StatusBadge status={rule.status} />
                    </div>
                    <p className="mt-3 text-xs font-semibold text-zinc-500">{rewardDefinitionLabel(version?.reward_definition)}</p>
                  </Link>
                );
              })
            )}
        ...
        <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-fuchsia-300">PRÓXIMOS HITOS</p>
          ...
          <div className="mt-5 grid gap-3">
            {!(cyclesResult.data ?? []).length ? (
              ...
              (cyclesResult.data ?? []).slice(0, 6).map((cycle) => (
                <Link key={cycle.id} href={`/admin/recompensas/reglas/${cycle.rule_id}/participantes/${cycle.student_id}`} className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/15 px-4 py-3">
                  <div>
                    <strong className="text-sm text-white">Ciclo en progreso</strong>
                    <p className="mt-1 text-xs text-zinc-500">{cycle.window_end_at ? `Cierra ${formatDateTime(cycle.window_end_at)}` : "Sin cierre fijo"}</p>
                  </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-fuchsia-300">ACTIVIDAD RECIENTE</p>
        ...
        <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-fuchsia-300">RECOMPENSAS</p>
          ...
          <div className="mt-5 grid gap-2">
            {!expiringRewards.length ? (
              ...
              expiringRewards.map((reward) => (
                <div key={reward.id} className="rounded-xl border border-white/10 bg-black/15 px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    <strong className="text-sm text-white">{rewardDefinitionLabel(reward.benefit_definition)}</strong>
                    <StatusBadge status={reward.status} />
                  </div>
                  <p className="mt-1 text-xs text-zinc-500">Vence {formatDateTime(reward.expires_at)}</p>
                </div>
              ))
            )}
          </div>
