...
import {
  ...
  waitlistPromotedCandidateKey,
  waitlistPromotedIdempotencyKey,
  WAITLIST_PROMOTED_CATALOG_CODE,
  WAITLIST_PROMOTED_CONSUMER_KEY,
  WAITLIST_PROMOTED_TEMPLATE,
  type MessagingProviderInput,
} from "./waitlist-promoted.ts";

type ProcessWaitlistPromotedRequest = {
  eventId?: unknown;
};
...
async function claimEvent(adminClient: SupabaseClient, eventId: string): Promise<boolean> {
  const { error } = await adminClient.rpc("claim_domain_event", {
    p_event_id: eventId,
    p_consumer_key: WAITLIST_PROMOTED_CONSUMER_KEY,
  });
  return !error;
}
...
async function recordEligibility(
  ...
  input: {
    ...
    occurredAt: string;
    eventId: string;
    conditions: ReturnType<typeof buildWaitlistPromotedConditions>;
  ...
  const { data, error } = await adminClient.rpc("record_automation_eligibility_evaluation", {
    ...
    p_candidate_key: waitlistPromotedCandidateKey(input.reservationId),
...
async function existingExecution(
  ...
  const { data } = await adminClient
    ...
    .eq("idempotency_key", waitlistPromotedIdempotencyKey(reservationId))
    .maybeSingle();
...
const handler = {
  fetch: withSupabase({ auth: "none" }, async (request, context) => {
    ...
    let payload: ProcessWaitlistPromotedRequest;
    try {
      payload = (await request.json()) as ProcessWaitlistPromotedRequest;
    } catch {
    ...
    if (
      dispatchEvent.event_type !== "booking.created" ||
    ...
    const eventPayload =
      contextData.event.payload && typeof contextData.event.payload === "object"
        ...
        : {};
    const eventSource = safeText(eventPayload.source);
    ...
    if (execution) {
      ...
      const { data: activeInstances, error: instanceError } = await adminClient
        ...
        .eq("catalog_code", WAITLIST_PROMOTED_CATALOG_CODE)
        .eq("status", "active")
        .limit(2);
      ...
      const { data: executionResult, error: executionError } = await adminClient.rpc(
        "system_create_automation_execution",
        {
          ...
          p_idempotency_key: waitlistPromotedIdempotencyKey(reservationId),
          p_data_snapshot: {
          ...
          p_template_snapshot: {
            catalog_code: WAITLIST_PROMOTED_CATALOG_CODE,
            template: WAITLIST_PROMOTED_TEMPLATE,
          },
    ...
    const errorMessage =
      providerResult.status === "skipped"
        ? "El webhook de Asistian no está configurado para waitlist_promoted."
        : "Asistian no aceptó la promoción desde lista de espera.";
