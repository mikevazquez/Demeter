...
revoke all on function public.student_book_session_with_resource(uuid, uuid)
from public, anon;
grant execute on function public.student_book_session_with_resource(uuid, uuid)
to authenticated;

create or replace function public.student_session_resource_map(
  target_session_id uuid
)
returns jsonb
language plpgsql
...
revoke all on function public.student_session_resource_map(uuid)
from public, anon;
grant execute on function public.student_session_resource_map(uuid)
to authenticated;
...
revoke all on function public.student_session_detail(uuid)
from public, anon;
grant execute on function public.student_session_detail(uuid)
to authenticated;

comment on function public.student_book_session_with_resource(uuid, uuid) is
  'RECURSOS-01 atomically books the current student and reserves a resource when required.';
comment on function public.student_session_resource_map(uuid) is
  'RECURSOS-01 student-safe resource availability and canonical geometry with aggregate occupancy only.';
