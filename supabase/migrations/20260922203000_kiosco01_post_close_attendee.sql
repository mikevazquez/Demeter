-- KIOSCO-01 · UAT Case 16
-- Administración puede registrar una asistencia real después del cierre automático.
-- La alta queda como reserva formal en estado attended, aplica cobertura comercial
-- cuando existe y emite una procedencia auditable.

create or replace function public.admin_add_post_close_attendee(
...
revoke all on function public.admin_add_post_close_attendee(uuid, uuid)
from public, anon;

grant execute on function public.admin_add_post_close_attendee(uuid, uuid)
to authenticated;
