-- SF-253: allow loyalty-backed challenges and achievements explicitly.
CREATE OR REPLACE FUNCTION public.system_compute_reward_loyalty_state(p_rule_id uuid, p_version_number integer, p_student_id uuid, p_as_of_date date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  if v_version.family <> 'loyalty'
     and not (
       v_version.family in ('challenge','achievement')
       and coalesce(v_version.presentation_definition->>'condition_family','') = 'loyalty'
     ) then
