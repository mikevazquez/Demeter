...
begin
  ...
  if v_is_single then
    ...
    if v_product.product_type::text in ('package','membership') then
      ...
      if not found or v_policy.enrollment_product_template_id is distinct from v_product.id then
...
begin
  ...
  select pt.product_type::text into v_main_product_type
  from public.product_templates pt
  where pt.id=v_attempt.product_template_id
    and pt.studio_id=v_attempt.studio_id;

  if v_main_product_type is null then
    raise exception 'product_not_available_online';
  end if;
  ...
  if v_main_product_type='enrollment' then
