create or replace function private.refresh_student_profile_status(p_student_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_studio_id uuid;
  v_person_id uuid;
  v_complete boolean;
begin
  select s.studio_id, s.person_id
  into v_studio_id, v_person_id
  from public.students s
  where s.id = p_student_id;

  if v_studio_id is null or v_person_id is null then
    return;
  end if;

  select not exists (
    select 1
    from public.profile_field_definitions d
    where d.studio_id = v_studio_id
      and d.entity_type = 'student'
      and d.active = true
      and d.required = true
      and (
        case d.key
          when 'first_name' then not exists (
            select 1 from public.persons p
            where p.id = v_person_id and btrim(coalesce(p.first_name, '')) <> ''
          )
          when 'last_name' then not exists (
            select 1 from public.persons p
            where p.id = v_person_id and btrim(coalesce(p.last_name, '')) <> ''
          )
          when 'phone' then not exists (
            select 1 from public.person_contacts c
            where c.person_id = v_person_id and c.kind = 'phone' and btrim(c.value) <> ''
          )
          when 'email' then not exists (
            select 1 from public.person_contacts c
            where c.person_id = v_person_id and c.kind = 'email' and btrim(c.value) <> ''
          )
          else not exists (
            select 1
            from public.profile_field_values v
            where v.definition_id = d.id
              and v.person_id = v_person_id
              and v.value <> 'null'::jsonb
              and not (
                jsonb_typeof(v.value) = 'string' and btrim(v.value #>> '{}') = ''
              )
              and not (
                jsonb_typeof(v.value) = 'array' and jsonb_array_length(v.value) = 0
              )
          )
        end
      )
  ) into v_complete;

  update public.students
  set profile_status = case
        when v_complete then 'complete'::public.profile_completeness_status
        else 'incomplete'::public.profile_completeness_status
      end,
      updated_at = now()
  where id = p_student_id;
end;
$$;

revoke all on function private.refresh_student_profile_status(uuid) from public;
revoke all on function private.refresh_student_profile_status(uuid) from anon;
revoke all on function private.refresh_student_profile_status(uuid) from authenticated;

create or replace function public.admin_set_student_profile_fields(
  p_student_id uuid,
  p_values jsonb
)
returns void
language plpgsql
set search_path = ''
as $$
declare
  v_studio_id uuid;
  v_person_id uuid;
  v_key text;
  v_value jsonb;
  v_definition public.profile_field_definitions%rowtype;
begin
  if jsonb_typeof(coalesce(p_values, '{}'::jsonb)) <> 'object' then
    raise exception 'profile_values_invalid';
  end if;

  select s.studio_id, s.person_id
  into v_studio_id, v_person_id
  from public.students s
  where s.id = p_student_id
  limit 1;

  if v_studio_id is null or v_person_id is null
     or not private.has_capability(v_studio_id, 'students.write') then
    raise exception 'students_write_denied';
  end if;

  for v_key, v_value in
    select e.key, e.value from jsonb_each(coalesce(p_values, '{}'::jsonb)) e
  loop
    if v_key = any(array['first_name','last_name','phone','email']) then
      raise exception 'structural_field_not_dynamic';
    end if;

    select d.* into v_definition
    from public.profile_field_definitions d
    where d.studio_id = v_studio_id
      and d.entity_type = 'student'
      and d.key = v_key
      and d.active = true
    limit 1;

    if not found then
      raise exception 'profile_field_unknown';
    end if;

    if v_value <> 'null'::jsonb then
      if v_definition.field_type in ('short_text','long_text','date','single_select')
         and jsonb_typeof(v_value) <> 'string' then
        raise exception 'profile_field_type_invalid';
      elsif v_definition.field_type = 'number'
         and jsonb_typeof(v_value) <> 'number' then
        raise exception 'profile_field_type_invalid';
      elsif v_definition.field_type = 'boolean'
         and jsonb_typeof(v_value) <> 'boolean' then
        raise exception 'profile_field_type_invalid';
      elsif v_definition.field_type = 'multi_select'
         and jsonb_typeof(v_value) <> 'array' then
        raise exception 'profile_field_type_invalid';
      end if;
    end if;

    if v_value = 'null'::jsonb
       or (jsonb_typeof(v_value) = 'string' and btrim(v_value #>> '{}') = '')
       or (jsonb_typeof(v_value) = 'array' and jsonb_array_length(v_value) = 0) then
      delete from public.profile_field_values
      where definition_id = v_definition.id and person_id = v_person_id;
    else
      insert into public.profile_field_values(definition_id, person_id, studio_id, value)
      values(v_definition.id, v_person_id, v_studio_id, v_value)
      on conflict (definition_id, person_id)
      do update set value = excluded.value, updated_at = now();
    end if;
  end loop;

  perform private.refresh_student_profile_status(p_student_id);
end;
$$;

revoke all on function public.admin_set_student_profile_fields(uuid, jsonb) from public;
revoke all on function public.admin_set_student_profile_fields(uuid, jsonb) from anon;
grant execute on function public.admin_set_student_profile_fields(uuid, jsonb) to authenticated;

create or replace function public.admin_create_student(
  p_first_name text,
  p_last_name text,
  p_phone text,
  p_email text default null
)
returns uuid
language plpgsql
set search_path = ''
as $$
declare
  v_studio_id uuid;
  v_person_id uuid;
  v_student_id uuid;
  v_full_name text;
begin
  select m.studio_id into v_studio_id
  from public.studio_memberships m
  where m.user_id = (select auth.uid())
    and m.active = true
    and private.has_capability(m.studio_id, 'students.write')
  limit 1;

  if v_studio_id is null then raise exception 'students_write_denied'; end if;
  if trim(coalesce(p_first_name, '')) = '' then raise exception 'first_name_required'; end if;
  if p_phone !~ '^\+[1-9][0-9]{7,14}$' then raise exception 'phone_invalid'; end if;

  if exists (
    select 1 from public.person_contacts
    where studio_id = v_studio_id and kind = 'phone' and value = p_phone
  ) then raise exception 'phone_exists'; end if;

  insert into public.persons(studio_id, first_name, last_name)
  values(v_studio_id, trim(p_first_name), nullif(trim(coalesce(p_last_name, '')), ''))
  returning id into v_person_id;

  insert into public.person_contacts(person_id, studio_id, kind, value, is_primary)
  values(v_person_id, v_studio_id, 'phone', p_phone, true);

  if nullif(trim(coalesce(p_email, '')), '') is not null then
    insert into public.person_contacts(person_id, studio_id, kind, value, is_primary)
    values(v_person_id, v_studio_id, 'email', lower(trim(p_email)), true);
  end if;

  v_full_name := trim(p_first_name || case
    when nullif(trim(coalesce(p_last_name, '')), '') is not null then ' ' || trim(p_last_name)
    else '' end);

  insert into public.students(
    studio_id, person_id, full_name, phone, email, active, lifecycle_status, profile_status
  ) values (
    v_studio_id, v_person_id, v_full_name, p_phone,
    nullif(lower(trim(coalesce(p_email, ''))), ''), true, 'active', 'incomplete'
  ) returning id into v_student_id;

  perform private.refresh_student_profile_status(v_student_id);
  return v_student_id;
end;
$$;

create or replace function public.admin_update_student(
  p_student_id uuid,
  p_first_name text,
  p_last_name text,
  p_phone text,
  p_email text default null
)
returns void
language plpgsql
set search_path = ''
as $$
declare
  v_studio_id uuid;
  v_person_id uuid;
  v_full_name text;
begin
  select s.studio_id, s.person_id into v_studio_id, v_person_id
  from public.students s where s.id = p_student_id limit 1;

  if v_studio_id is null or v_person_id is null
     or not private.has_capability(v_studio_id, 'students.write') then
    raise exception 'students_write_denied';
  end if;

  if trim(coalesce(p_first_name, '')) = '' then raise exception 'first_name_required'; end if;
  if p_phone !~ '^\+[1-9][0-9]{7,14}$' then raise exception 'phone_invalid'; end if;

  if exists (
    select 1 from public.person_contacts
    where studio_id = v_studio_id and kind = 'phone' and value = p_phone and person_id <> v_person_id
  ) then raise exception 'phone_exists'; end if;

  update public.persons
  set first_name = trim(p_first_name),
      last_name = nullif(trim(coalesce(p_last_name, '')), ''),
      updated_at = now()
  where id = v_person_id;

  update public.person_contacts
  set value = p_phone, is_primary = true, updated_at = now()
  where person_id = v_person_id and kind = 'phone';
  if not found then
    insert into public.person_contacts(person_id, studio_id, kind, value, is_primary)
    values(v_person_id, v_studio_id, 'phone', p_phone, true);
  end if;

  if nullif(trim(coalesce(p_email, '')), '') is null then
    delete from public.person_contacts where person_id = v_person_id and kind = 'email';
  else
    update public.person_contacts
    set value = lower(trim(p_email)), is_primary = true, updated_at = now()
    where person_id = v_person_id and kind = 'email';
    if not found then
      insert into public.person_contacts(person_id, studio_id, kind, value, is_primary)
      values(v_person_id, v_studio_id, 'email', lower(trim(p_email)), true);
    end if;
  end if;

  v_full_name := trim(p_first_name || case
    when nullif(trim(coalesce(p_last_name, '')), '') is not null then ' ' || trim(p_last_name)
    else '' end);

  update public.students
  set full_name = v_full_name,
      phone = p_phone,
      email = nullif(lower(trim(coalesce(p_email, ''))), ''),
      updated_at = now()
  where id = p_student_id;

  perform private.refresh_student_profile_status(p_student_id);
end;
$$;

do $$
declare r record;
begin
  for r in select id from public.students loop
    perform private.refresh_student_profile_status(r.id);
  end loop;
end;
$$;
