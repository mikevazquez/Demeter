...
begin
  ...
  v_full_name := trim(
    p_first_name ||
    case when nullif(trim(coalesce(p_last_name, '')), '') is not null
      then ' ' || trim(p_last_name) else '' end
  );
...
begin
  ...
  loop
    ...
    if not found then raise exception 'profile_field_unknown'; end if;
    ...
    if v_value <> 'null'::jsonb then
      if v_definition.field_type in ('short_text','long_text','date','single_select')
        ...
        raise exception 'profile_field_type_invalid';
      elsif v_definition.field_type = 'multi_select' and jsonb_typeof(v_value) <> 'array' then
        raise exception 'profile_field_type_invalid';
      end if;
    end if;
...
revoke all on function public.admin_create_student(text,text,text,text) from public, anon;
revoke all on function public.admin_update_student(uuid,text,text,text,text) from public, anon;
revoke all on function public.admin_set_student_profile_fields(uuid,jsonb) from public, anon;
