...
begin
  ...
  loop
    ...
    if coalesce(jsonb_typeof(v_condition->'applies'), '') <> 'boolean'
       or coalesce(jsonb_typeof(v_condition->'passed'), '') <> 'boolean' then
      raise exception 'automation_eligibility_condition_boolean_required';
    end if;

    if v_condition->>'kind' = 'protected'
       and (v_condition->>'applies')::boolean = false then
...
declare
  ...
  v_eligible boolean;
  v_decision public.automation_eligibility_decision;
...
begin
  ...
  select coalesce(
    jsonb_agg(
      ...
      where (condition->>'applies')::boolean
        and not (condition->>'passed')::boolean
    ),
    '[]'::jsonb
  )
  ...
  v_decision := case
    when v_eligible then 'eligible'::public.automation_eligibility_decision
    else 'ineligible'::public.automation_eligibility_decision
  end;
